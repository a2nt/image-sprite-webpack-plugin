 ...
 *
 * Copyright (c) 2019 Tony Air <tony@twma.pro>
...
function hasDeclaration(declarations, property, value) {
    return declarations.some(declaration => {
        return declaration.property === property && declaration.value === value;
    });
}
...
class ImageSpritePlugin {
    constructor(option) {
        ...
        this._outputFilename =
            option.outputFilename || '/sprite/sprite-[hash].png';
        this._compress =
        ...
        this._sprite = null;
        this._exclude = option.exclude || null;
    ...
    createCssAstMap(assetName, asset) {
        if (isConcatSource(asset)) {
            ...
            asset.children.forEach((/*SourceMapSource*/ childAsset, i) => {
                this._cssAstMap[assetName][i] = css.parse(childAsset.source(), {
                    source: `source${i}.css`,
                });
            });
        } else {
    ...
    getSpriteSources(compilation) {
        const sources = [];
        if (this.isInlineCss(compilation)) {
            this.eachImageAssets(compilation, (
                assetName,
                /*RawSource*/ asset,
            ) => {
                sources.push(
    ...
    getSpriteSourcesFromCssAssets(compilation) {
        ...
        this.eachCssAssets(compilation, (
            cssAssetName,
            /*ConcatSource*/ cssAsset,
        ) => {
            this.createCssAstMap(cssAssetName, cssAsset);
            if (
                isConcatSource(cssAsset) &&
                Array.isArray(this._cssAstMap[cssAssetName])
            ) {
                this._cssAstMap[cssAssetName].forEach(ast => {
                    const /*Set*/ childSources = this.getSpriteSourceSetFromAst(
                            ast,
                            candidates,
                        );
    ...
    isImage(fileName) {
        if (typeof fileName === 'string') {
            if (fileName.match(this._exclude)) {
                return false;
            }
    ...
    transformJsAsset(asset, name, compilation, imageModules) {
        ...
        imageModules.forEach(mod => {
            this.updateJsBackgroundShorthand(
                compilation,
                replaceable,
                name,
                mod,
        ...
        while (occurrence > -1) {
            ...
            if (bgImage) {
                ...
                if (
                    ...
                    this.addPositionComment(
                        bgPosition.declaration,
                        bgPosition.index + 1,
                    );
                    const cssSource = this.getCssSource(ast);
                    const toBe = cssSource
                        .replace(PSEUDO_CLASS, '')
                        .replace(/\n/g, EOL);
                    replaceable.replace(start, end, toBe);
                    logger.transformOk(
                        getBackgroundShorthand(rawRequest),
                        newImgVal,
                    );
                    logger.transformOk(
                        'background-position: 0 0',
        ...
        while (occurrence > -1) {
            ...
            if (propertyMap['background']) {
                ...
                if (!declaration.value.includes('no-repeat')) {
                    ...
                    const newVal = `url(${this.getOutFilePath()}) no-repeat ${getPosition(
                        coord,
                    )}`;
